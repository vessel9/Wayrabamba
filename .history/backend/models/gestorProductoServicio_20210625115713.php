...
class GestorProductoServicioModel{
	...
	#MOSTRAR NEGOCIO
	#------------------------------------------------------
	public function mostrarProductoServicioModel($tabla1, $tabla2, $tabla1){

		$stmt = Conexion::conectar()->prepare("SELECT idproductoServicio, nombreProductoServicio, descripcion, imagenRuta, precio, cantidad, disponibles, nombreNegocio, nombreTipo, nombreCategoria FROM productoservicio AS ps INNER JOIN negocio AS n ON ps.idNegocio= n.idNegocio INNER JOIN tipoproductoservicio AS tps ON ps.idTipoProductoServicio = tps.idTipoProductoServicio INNER JOIN categorias AS c ON ps.idNegocio= c.idCategoria");
