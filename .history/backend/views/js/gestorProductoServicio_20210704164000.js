...
// $(".editarProductoServicio").click(function(){
	...
	$(document).ready(function(){
	 ...
	 $(".cambiarImagen").click(function(){
		...
		$(this).hide();
		$("#subirNuevaFoto"+imagenRuta).show();
		$("#subirNuevaFoto"+imagenRuta).css({"width":"90%"});
		$("#nuevaFoto"+imagenRuta).html("");
		$("#subirNuevaFoto"+imagenRuta).attr("name","editarImagen");
		$("#subirNuevaFoto"+imagenRuta).attr("required",true);
		$("#subirNuevaFoto"+imagenRuta).change(function(){

			imagen = this.files[0];
			imagenSize = imagen.size;
			if(Number(imagenSize) > 2000000){

				$("#editarImagen"+imagenRuta).before('<div class="alert alert-warning alerta text-center">El archivo excede el peso permitido, 200kb</div>')
			}
			else{
				$(".alerta").remove();
			}
			...
			if(imagenType == "image/jpeg" || imagenType == "image/png"){
				...
				$("#editarImagen"+imagenRuta).before('<div class="alert alert-warning alerta text-center">El archivo debe ser formato JPG o PNG</div>')
			}
			if(Number(imagenSize) < 2000000 && imagenType == "image/jpeg" || imagenType == "image/png"){
				...
				$.ajax({
						...
						beforeSend: function(){
							...
							$("#nuevaFoto"+imagenRuta).html('<img src="views/images/status.gif" style="width:15%" id="status2">');
						},
						success: function(respuesta){
							$("#status2"+imagenRuta).remove();
							if(respuesta == 0){

								$("#editarImagen"+imagenRuta).before('<div class="alert alert-warning alerta text-center">La imagen es inferior a 800px * 400px</div>')
							}
							else{
								$("#nuevaFoto"+imagenRuta).html('<img src="'+respuesta.slice(6)+'" class="img-thumbnail">');
							}	
						}
				})	
			}
