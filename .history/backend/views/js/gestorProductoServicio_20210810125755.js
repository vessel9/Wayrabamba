		...
		$.ajax({
			...
			success: function(respuesta){
					...
					if(respuesta == 0){
						...
						$("#arrastreImagenProductoServicio").before('<div class="alert alert-warning alerta text-center">Se recomienda la imagen de 800px * 400px, esta es inferior pero se guaradara</div>')
...
// 	nombreProductoServicio = $("#"+idProductoServicio).children("h1").html();
	...
	// $("#"+idProductoServicio).html('<div><form method="post" enctype="multipart/form-data"><span><input style="width:10%; padding:5px 0; margin-top:4px" type="button" class="btn btn-danger pull-right" value="Cancelar" data-dismiss="modal"  onClick="cierraProductoServicios()"><input style="width:10%; padding:5px 0; margin-top:4px" type="submit" class="btn btn-primary pull-right" value="Actualizar imagen"></span><div id="editarImagen"><input st/yle="display:none" type="file" id="subirNuevaFoto" class="btn btn-default"><div id="nuevaFoto"><span class="fa fa-times cambiarImagen"></span><img src="'+imagenRuta+'" class="img-thumbnail"></div></div><input type="hidden" value="'+idProductoServicio+'" name="editarIdproductoServicio"><input type="text" value="'+nombreProductoServicio+'" ><input type="hidden" value="'+imagenRuta+'" name="fotoAntigua"><hr></form></div>');
