...
<div id="seccionArticulos" class="col-lg-10 col-md-10 col-sm-9 col-xs-12">
	...
	<div id="agregarProductoServicio" style="display:none">
		...
		<form method="post" enctype="multipart/form-data">
			...
			<select name="Equipment1" id = "idTipoProductoServicio"> 
<?php
require_once "conexion.php";;
$query = "SELECT idTipoProductoServicio, nombreTipo FROM $tabla ORDER BY idTipoProductoServicio ASC";
    $data = $odb->prepare($query);    // Prepare query for execution
    $data->execute();// Execute (run) the query

    while($row=$data->fetch(PDO::FETCH_ASSOC)){
        echo '<option value="'.$row['equipment_id'].'">'.$row['idTipoProductoServicio'].'</option>';
        //print_r($row); 
    }
?>
 </select>
