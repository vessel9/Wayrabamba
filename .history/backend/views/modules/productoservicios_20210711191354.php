...
<div id="seccionArticulos" class="col-lg-10 col-md-10 col-sm-9 col-xs-12">
	...
	<div id="agregarProductoServicio" style="display:none">
		...
		<form method="post" enctype="multipart/form-data">
			...
			<?php	
			if($_SESSION["rol"] == 0){?>
				             <select name="editarEstado" class="form-control" required>';
							 <option value="<?php'.$item["estado"]?>">Selecciona EStado</option>';
							 <option value="publico">Publico</option>
							 <option value="privado">Privado</option>

						  </select>				
							  else{
			?>					<input type="hidden" value="<?php $item["estado"] ?>" name="editarEstado">
		}
			</div>	
