...
?>
		...
		<select name="nuevoRol" class="form-control" required><option value="">Seleccione el Rol</option><?php foreach($respuesta as $row => $item){?><option value="0"><?php echo $item["nombreTipo"] ?></option>
		<?php }
		?>
       </select>
	</div>
