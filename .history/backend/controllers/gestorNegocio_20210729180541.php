		...
		foreach($respuesta as $row => $item) {
			...
			<li class="card" id="'.$item["idNegocio"].'" class="bloqueArticulo" style="max-width:100%;width:800;height:400;">
					<span class="handleArticle">
					 <a href="index.php?action=negocios&idConfirmarNegocio='.$item["idNegocio"].'&confimagenNegocio='.$item["imagenNegocio"].'">
	...
	public function confirmarBorrarNegocioController(){
		...
		if(isset($_GET["idConfirmarNegocio"])){
			...
			foreach($respuesta as $row => $item) {
					...
					function(isConfirm){
							 if (isConfirm) {							
								window.location.href="index.php?action=negocios&idBorrarNegocio='.$item["idNegocio"].'&imagenNegocio='.$item["imagenNegocio"].'";							
							  } else {
