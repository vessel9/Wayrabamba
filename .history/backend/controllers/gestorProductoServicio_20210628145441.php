...
class GestorProductoServicio{
	...
	public function mostrarProductoServicioController2(){
		...
		var_dump($respuesta);
		foreach($respuesta as $row => $item) {
			echo'<select name="idTipoProductoServicio" class="form-control" required>
			<?php
	    $mostrarProductoServicio2 = new GestorProductoServicio(); 
		$mostrarProductoServicio2 -> mostrarProductoServicioController2();
		var_dump($respuesta);
		foreach($respuesta as $row => $item) {

	?>
            <option value=><?php echo $item['nombreTipo']?></option>
			<?php
		}
			?>';
		}
