...
class GestorProductoServicio{
	...
	public function guardarProductoServicioController(){
		...
		$datosController = array("nombreProductoServicio"=>ucfirst($_POST["nombreProductoServicio"]),
	...
	public function mostrarProductoServicioController(){
		...
		foreach($respuesta as $row => $item) {
			...
			echo ' <li id="'.$item["idproductoServicio"].'" class="bloqueNegocio">
					...
					<a href="index.php?action=productoservicios&idConfirmarProductoServicio='.$item["idproductoServicio"].'&confirmarutaImagen='.$item["imagenRuta"].'">
					...
					<h3 class="modal-title">Editar Imagen de Producto o Servicio</h3></div>';
				...
				<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
					...
					echo '<div class="modal-body">
					    ...
					    <input id="modal" name="editarNombreProductoServicio" type="text" placeholder="Nombre producto" class="form-control" value="'.$item["nombreProductoServicio"].'" required>
						</div>
						<div class="form-group">
						<label>Descripción</label>
						<input id="modal" name="editarDescripcion" type="text" placeholder="descripcion" class="form-control" value="'.$item["descripcion"].'"required>
						</div>
						<div class="form-group">
						<label>Precio</label>
						<input id="modal" name="editarPrecio" type="text" placeholder="precio" class="form-control" value="'.$item["precio"].'"required>
						</div>
						<div class="form-group">
						<label>Cantidad</label>
						<input id="modal" name="editarCantidad" type="text" placeholder="cantidad" class="form-control" value="'.$item["cantidad"].'"required>
						</div>
						<div class="form-group">
						<label>Disponibilidad</label>
						<input id="modal" name="editarDisponibles" type="text" placeholder="disponibles" class="form-control" value="'.$item["disponibles"].'" required></div>';
						
						echo '<label>Tipo</label><select editarProductoServicio name="editarIdTipoProductoServicio" class="form-control">
						...
						echo '<label>Categoria</label><select name="editarIdCategoria" class="form-control">
						...
						echo'<label>Negocio</label><select name="editarIdNegocio" class="form-control">
						...
						echo'</select>';
							if($_SESSION["rol"] == 0){
				        echo'<label>Estado</label><select name="editarEstado" class="form-control">';
		...
		if(isset($_GET["idConfirmarProductoServicio"])){
			$respuesta = GestorProductoServicioModel::mostrarProductoServicioModel("productoservicio", "negocio", "tipoproductoservicio", "categorias");
				foreach($respuesta as $row => $item) {
					...
					function(isConfirm){
							 if (isConfirm) {							
								window.location.href="index.php?action=productoservicios&idBorrar='.$_GET["idConfirmarProductoServicio"].'&rutaImagen='.$_GET["confirmarutaImagen"].'";							
							  } else {
	...
	public function editarProductoServicioController(){
		...
		if(isset($_POST["editarNombreProductoServicio"])){
			$datosController = array( "idproductoServicio"=>$_POST["idProductoServicio"],
				                      "nombreProductoServicio"=>ucfirst($_POST["editarNombreProductoServicio"]),
