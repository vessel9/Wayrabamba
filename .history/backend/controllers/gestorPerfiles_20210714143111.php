...
class GestorPerfiles{
	...
	public function guardarPerfilController(){
		...
		if(isset($_POST["nuevoUsuario"])){	
			...
			if(isset($_FILES["nuevaImagen"]["tmp_name"])){
			    ...
			    $borrar = glob("views/images/perfiles/temp/*");
		...
		foreach($respuesta as $row => $item){
			...
			echo ' <tr>
			        ...
			        <td><a href="#perfil'.$item["id"].'" data-toggle="modal"><span class="btn btn-info fa fa-pencil"></span></a>
			            <a href="index.php?action=perfil&idConfirmarPerfil='.$item["id"].'&rolConfirma='.$item["rol"].'"><span class="btn btn-danger fa fa-times"></span></a></td>
			      </tr>
								     ...
								     <div class="form-group">
								      ...
								      <div class="form-group">
								       ...
								       <div class="form-group text-center">
							    		...
							    		<input type="file" class="btn btn-default" name="editarImagen" style="display:inline-block; margin:10px 0" required>
	...
	#------------------------------------
	public function confirmarBorrarPerfilController(){
		


		if(isset($_GET["idConfirmarPerfil"])){
			$respuesta = GestorPerfilesModel::verPerfilesModel("usuarios");	
			$datosController = $_GET["idConfirmarPerfil"];
			$datosController2 = $_GET["rolConfirma"];

			$respuesta2 = GestorPerfilesModel::verPerfilesModel2($datosController, "usuarios", $datosController2);	

                if($datosController = $_GET["idConfirmarPerfil"] && $datosController2 = $_GET["rolConfirma"]){
					echo'<script>

						swal({
							  title: "¡OK!",
							  text: "¡No se puede eliminar administrador",
							  type: "success",
							  confirmButtonText: "Cerrar",
							  closeOnConfirm: false
						},
					...
					function(isConfirm){
							 if (isConfirm) {							
								window.location.href="index.php?action=perfil&idBorrar='.$item["id"].'&borrarImg='.$item["photo"].'";							
							  } else {
								event.preventDefault();
								swal("Cancelled", "perfil a salvo :)", "error");
								setTimeout(()=>{
									window.location = "perfil";
								},1000);
							  }
	...
	public function borrarPerfilController(){
		...
		if(isset($_GET["idBorrar"])){
			...
			if($respuesta == "ok"){
					...
					swal({
						  title: "¡OK!",
						  text: "¡Si el usuario era Editor se ha borrado exitosamente!",
						  type: "success",
						  confirmButtonText: "Cerrar",
						  closeOnConfirm: false
					},
