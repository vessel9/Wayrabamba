...
class GestorSlide{
	...
	public function mostrarImagenVistaController(){
		...
		foreach($respuesta as $row => $item){
			...
			echo '<li id="'.$item["id"].'" class="bloqueSlide"><span class="fa fa-times" href="index.php?action=slide&idConfirmar"></span><img src="'.substr($item["ruta"], 6).'" class="handleImg"></li>';
	...
	public function confirmarBorrarArticuloController(){
		if(isset($_GET["idConfirmar"])){
			...
			foreach($respuesta as $row => $item) {
					...
					function(isConfirm){
							 if (isConfirm) {							
								window.location.href="eliminarSlide";							
							  } else {
