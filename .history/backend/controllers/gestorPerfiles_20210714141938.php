...
class GestorPerfiles{
	...
	public function verPerfilesController(){
		...
		foreach($respuesta as $row => $item){
			...
			echo ' <tr>
			        ...
			        <td>'.$rol.'</td>
			        <td>'.$item["email"].'</td>
			        <td><a href="#perfil'.$item["id"].'&borrarImg='.$item["photo"].'" data-toggle="modal"><span class="btn btn-info fa fa-pencil"></span></a>
			            <a href="index.php?action=perfil&idConfirmarPerfil"><span class="btn btn-danger fa fa-times"></span></a></td>
			      </tr>
	...
	public function confirmarBorrarPerfilController(){
		...
		if(isset($_GET["idConfirmarPerfil"])){
			...
			$respuesta2 = GestorPerfilesModel::verPerfilesModel2($datosController, "usuarios");	

                if($respuesta2 ==$_SESSION["rol"]){
					echo'<script>
