...
class GestorPerfiles{
	...
	public function confirmarBorrarPerfilController(){
		...
		if(isset($_GET["idConfirmarPerfil"])){
			...
			$datosController2 = $_GET["idConfirmarPerfil"];

			$respuesta = GestorPerfilesModel::verPerfilesModel2($datosController, "usuarios");	
			$respuesta2 = GestorPerfilesModel::verPerfilesModel3($datosController2, "usuarios");	
				if($respuesta == "ok" && $respuesta2 == false ){
					echo'<script>

					swal({
						  title: "¡OH!",
						  text: "¡No se puede borrar Admisnistradores!",
						  type: "success",
						  confirmButtonText: "Cerrar",
						  closeOnConfirm: false
					},
